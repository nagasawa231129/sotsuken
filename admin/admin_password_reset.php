    ...
    if (!empty($to)) {
        try {
            ...
            if ($user) {
                ...
                // トークンと有効期限をデータベースに保存
                $stmt = $dbh->prepare("UPDATE admin SET reset_token = :token, reset_expires = :expires WHERE admin_id = :id");
                $stmt->execute(['token' => $token, 'expires' => $expires_at, 'id' => $user['admin_id']]);

                // リセットリンクの生成
                $reset_link = "https://y231129.daa.jp/sotsuken/sotsuken/user/password_update.php?token=" . $token . "&admin_id=" . $user['admin_id'];
...
<head>
    ...
    <link rel="stylesheet" href="admin_password_reset.css">
